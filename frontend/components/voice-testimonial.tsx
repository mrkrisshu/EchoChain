  ...
  useEffect(() => {
    ...
    testimonials.forEach((testimonial) => {
      if (testimonial.audio) {
        ...
        const audioSrc = testimonial.audio.startsWith('http')
          ? testimonial.audio
      ...
      <div className="flex flex-col items-center justify-center pt-5">
        <div className="flex flex-col gap-5 mb-8">
          ...
          <span className="text-center text-slate-300">
            Real voice creators sharing their EchoChain success stories
          </span>
        </div>
      </div>
      <div className="relative">
        <div className={`flex justify-center items-start gap-5 flex-wrap shadow-black overflow-hidden ${showAll ? 'max-h-full' : 'max-h-[720px]'} relative`}>
          ...
          {testimonials.map((testimonial, index) => (
            <div
              ...
              className={`${mode === 'dark' ? 'bg-black' : 'bg-white'
                } border border-zinc-700 w-72 h-[280px] rounded-2xl p-5 relative flex flex-col ${!showAll && index >= 6 ? 'testimonial-partially-visible' : ''
                }`}>
